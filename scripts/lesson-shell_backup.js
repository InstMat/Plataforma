...
function parseParams() {
    ...
    window.lessonBase = urlParams.get('base') || '';
...
async function modifyTitleSlide() {
    ...
    try {
        ...
        // Extract course path and lesson from lessonBase
        const pathParts = window.lessonBase.split('/');
        const coursePath = pathParts.slice(0, -2).join('/'); // e.g., "FEN/Matematicas-AUD-CPA"
        const unitPath = pathParts[pathParts.length - 2]; // e.g., "UnidadI"
        const lessonPath = pathParts[pathParts.length - 1]; // e.g., "clase01"
        ...
        for (const [key, module] of Object.entries(carrerasData.modulos_comunes)) {
            if (module.enlace && module.enlace.includes(coursePath)) {
                moduleKey = key;
        ...
        // Load lecciones.json for this course
        const leccionesResponse = await fetch(`${coursePath}/lecciones.json`, { cache: 'no-cache' });
        const leccionesData = await leccionesResponse.json();

        // Find the lesson name
        let lessonName = null;
        for (const unidad of leccionesData.unidades) {
            for (const leccion of unidad.lecciones) {
                if (leccion.enlace.includes(`${unitPath}/${lessonPath}`)) {
